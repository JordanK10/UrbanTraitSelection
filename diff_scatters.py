...
def create_scatter_plot(data, x_var, y_var, title, filename, figsize=(10, 8)):
    ...
    # Save plot
    plt.tight_layout()
    plt.savefig(f'diff_scatter_plots/{filename}', dpi=300, bbox_inches='tight')
    plt.close()
    print(f"Saved: diff_scatter_plots/{filename}")
...
def create_all_plots(data):
    ...
    import os
    os.makedirs('diff_scatter_plots', exist_ok=True)
...
def main():
    ...
    print(f"Generated {total_plots} difference scatter plots in diff_scatter_plots/ directory")
