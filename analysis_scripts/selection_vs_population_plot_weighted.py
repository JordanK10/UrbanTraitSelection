...
OUTPUT_FILE = os.path.join(OUTPUT_DIR, 'log_per_capita_pop_weighted_abs_selection_vs_log_population.pdf')
...
def process_level_data(filepath, level_suffix, county_col='ParentCounty'):
    """
    Loads and aggregates data for a specific geographic level using population weighting.
    """
    ...
    # --- MODIFICATION: Calculate population-weighted average of absolute selection ---
    df['weighted_abs_selection'] = df[selection_col].abs() * df[pop_col]
    county_weighted_abs_selection_sum = df.groupby(county_col)['weighted_abs_selection'].sum()
    
    # Aggregate population
    county_pop = df.groupby(county_col)[pop_col].sum()

    # Divide the weighted sum by total population to get the weighted average
    county_weighted_avg_selection = county_weighted_abs_selection_sum / county_pop
    # --- END MODIFICATION ---

    df_county = pd.DataFrame({
        'total_pop': county_pop,
        'total_abs_selection': county_weighted_avg_selection
    }).reset_index()
...
def create_plot():
    """
    Generates a scatter plot comparing tract and community level 
    PER-CAPITA POPULATION-WEIGHTED selection vs. population.
    """
