...
def process_level(level, level_suffix):
    ...
    save_plot(fig_msa, os.path.join(BASE_OUTPUT_DIR, f'specialty_scatter_plots/specialty_scatter_full_msa_{level}.pdf'))
    ...
    for county_code in unique_counties:
        ...
        filename = os.path.join(BASE_OUTPUT_DIR, f'specialty_scatter_plots/specialty_scatter_{safe_county_name}_{level}.pdf')
...
def main():
    ...
    os.makedirs(os.path.join(BASE_OUTPUT_DIR, 'specialty_scatter_plots'), exist_ok=True)
    ...
    print("\nAll plots saved in: plots/specialty_scatter_plots/ directory")
