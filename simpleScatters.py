...
def create_scatter_plot(data, x_var, y_var, color_var, title, filename, figsize=(10, 8)):
    ...
    # Save plot
    plt.tight_layout()
    plt.savefig(f'simple_scatter_plots/{filename}', dpi=300, bbox_inches='tight')
    plt.close()
    print(f"Saved: simple_scatter_plots/{filename}")
...
def create_all_plots(data):
    ...
    import os
    os.makedirs('simple_scatter_plots', exist_ok=True)
...
def main():
    ...
    print(f"Generated {total_plots} scatter plots in simple_scatter_plots/ directory")
