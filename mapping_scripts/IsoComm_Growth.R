...
INCOME_GROWTH_MIN <- -0.05  # Manual minimum bound
INCOME_GROWTH_MAX <- 0.05   # Manual maximum bound
...
A_INCOME_GROWTH_MIN <- 10  # Manual minimum bound
A_INCOME_GROWTH_MAX <- 12   # Manual maximum bound
A_INCOME_GROWTH_AUTO_COLOR_SCHEME <- TRUE  # When TRUE, selects inferno if min > 0, custom scheme otherwise
A_INCOME_GROWTH_FORCE_INFERNO <- TRUE     # Force using inferno color scheme regardless of min value
A_INCOME_GROWTH_FORCE_CUSTOM <- FALSE      # Force using custom color scheme regardless of min value

# Map-specific settings
# Income Growth Rate Map Settings
A_POPULATION_USE_DATA_DRIVEN_LIMITS <- FALSE  # Set to FALSE to use manual limits
A_POPULATION_USE_PERCENTILE_LIMITS <- FALSE   # When TRUE, uses 10-90th percentiles; when FALSE uses min-max
A_POPULATION_MIN <- 500  # Manual minimum bound
A_POPULATION_MAX <- 100000   # Manual maximum bound
A_POPULATION_AUTO_COLOR_SCHEME <- TRUE  # When TRUE, selects inferno if min > 0, custom scheme otherwise
A_POPULATION_FORCE_INFERNO <- TRUE     # Force using inferno color scheme regardless of min value
A_POPULATION_FORCE_CUSTOM <- FALSE      # Force using custom color scheme regardless of min value
...
POP_CHANGE_MAX <- .05  # Manual maximum bound
POP_CHANGE_AUTO_COLOR_SCHEME <- FALSE  # When TRUE, selects inferno if min > 0, custom scheme otherwise
...
# Output file names
COVARIANCES_MAP_FILE <- "iso_growth.pdf"
...
create_variable_map <- function(data, variable, title, latex_title = NULL, latex_legend = NULL,
  ...
  p <- ggplot() +
    ...
    #       #filter(!community %in% c("LOWER WEST SIDE", "MCKINLEY PARK", "BRIDGEPORT")),
...
if(A_POPULATION_USE_DATA_DRIVEN_LIMITS) {
  cat("Using data-driven limits for Income Growth Rate\n")
  # Use map-specific percentile setting
  USE_PERCENTILE_LIMITS_TEMP <- A_POPULATION_USE_PERCENTILE_LIMITS
  
  a_population_limits <- analyze_data_distribution(
    map_data, 
    "PopInitial_cm", 
    additional_data, 
    "PopInitial_cm"
  ...
  cat("Using manual limits for Population Change\n")
  a_population_limits <- list(min = A_POPULATION_MIN, max = A_POPULATION_MAX)
}
...
required_vars <- c("cum_sel_inc_ratio",   "cum_sel_gro_ratio" )
...
frac_pop_change_map <- create_variable_map(
  map_data, 
  "PopG_cm",
  "ative Household Growth",
  latex_title = expression(paste("ative Household Growth")),
  latex_legend = "X",
...
income_growth_map <- create_variable_map(
  map_data, 
  "RelAvgG_emp_cm",
  "ative Average Income Growth",
  latex_title = expression(paste("ative Average Income Growth")),
  latex_legend = "X",
...
a_income_growth_map <- create_variable_map(
  map_data, 
  "LogAvgIncInitial_cm",
  "ative Aggregate Income Growth",
  latex_title = expression(paste("ative Aggregate Income Growth")),
  latex_legend = "X",
...
# Create the three maps with LaTeX notation and optimized color scales
a_population_map <- create_variable_map(
  map_data, 
  "PopInitial_cm",
  "ative Aggregate Income Growth",
  latex_title = expression(paste("ative Aggregate Income Growth")),
  ...
  max_limit = a_population_limits$max,
  min_limit = a_population_limits$min,
  auto_color_scheme = A_POPULATION_AUTO_COLOR_SCHEME,
  force_inferno = A_POPULATION_FORCE_INFERNO,
  force_custom = A_POPULATION_FORCE_CUSTOM,
  cap_values = TRUE,
  optimize_distribution = A_POPULATION_USE_DATA_DRIVEN_LIMITS
)
...
combined_map <-  frac_pop_change_map + income_growth_map + a_income_growth_map+a_population_map+
