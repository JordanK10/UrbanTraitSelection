...
COV_POP_TERM_USE_DATA_DRIVEN_LIMITS <- FALSE  # Set to FALSE to use manual limits
COV_POP_TERM_USE_PERCENTILE_LIMITS <- FALSE   # When TRUE, uses 10-90th percentiles; when FALSE uses min-max
COVARIANCE_MIN_POP <- -30  # Manual minimum bound
COVARIANCE_MAX_POP <- 25  # Manual maximum bound
COV_POP_TERM_AUTO_COLOR_SCHEME <- FALSE  # When TRUE, selects inferno if min > 0, custom scheme otherwise
COV_POP_TERM_FORCE_INFERNO <- FALSE     # Force using inferno color scheme regardless of min value
COV_POP_TERM_FORCE_CUSTOM <- TRUE      # Force using custom color scheme regardless of min value
...
COVARIANCE_MIN_INC <- -60  # Manual minimum bound
COVARIANCE_MAX_INC <- 20   # Manual maximum bound
...
# Output file names
DEMOGRAPHICS_MAP_FILE <- "output_maps/chicago_community_maps_demos.pdf"
...
# Covariance Term limits
if(COV_POP_TERM_USE_DATA_DRIVEN_LIMITS) {
  cat("Using data-driven limits for Covariance Term\n")
  # Use map-specific percentile setting
  USE_PERCENTILE_LIMITS_TEMP <- COV_POP_TERM_USE_PERCENTILE_LIMITS
  
  cov_pop_term_limits <- analyze_data_distribution(
    map_data, 
    "Cumulative_Sel_pop_PNC_ct", 
    additional_data, 
    "Cumulative_Sel_pop_PNC_ct"
  )
  ...
  cat("Using manual limits for Covariance Term\n")
  cov_pop_term_limits <- list(min = COVARIANCE_MIN_POP, max = COVARIANCE_MAX_POP)
}

# Print all the selected limits
cat("\nSelected limits for all maps:\n")
cat("Covariance gro: min =", cov_pop_term_limits$min, ", max =", cov_pop_term_limits$max, "\n")
...
required_vars <- c("Cumulative_Sel_inc_PNC_ct", "Cumulative_Sel_pop_PNC_ct")
...
price_cov_term_map <- create_variable_map(
  map_data, 
  "Cumulative_Sel_pop_PNC_ct", 
  "Cumulative Population Selection Effects (% of Average Growth)",
  latex_title = expression(paste("Cumulative Population Selection Effects (% of Average Growth)")),
  latex_legend = "X",
  max_limit = cov_pop_term_limits$max,
  min_limit = cov_pop_term_limits$min,
  auto_color_scheme = COV_POP_TERM_AUTO_COLOR_SCHEME,
  force_inferno = COV_POP_TERM_FORCE_INFERNO,
  force_custom = COV_POP_TERM_FORCE_CUSTOM,
  cap_values = TRUE,
  optimize_distribution = COV_POP_TERM_USE_DATA_DRIVEN_LIMITS
)
...
income_cov_term_map <- create_variable_map(
  map_data, 
  "Cumulative_Sel_inc_PNC_ct", 
  "Cumulative Income Selection Effects (% of Aggregate Growth)",
  latex_title = expression(paste("Cumulative Income Selection Effects (% of Aggregate Growth) ")),
  latex_legend = "X",
...
combined_map <-  price_cov_term_map  +income_cov_term_map + 
