...
INCOME_GROWTH_MIN <- -0.06  # Manual minimum bound
INCOME_GROWTH_MAX <- 0.1   # Manual maximum bound
...
A_INCOME_GROWTH_MIN <- 9  # Manual minimum bound
A_INCOME_GROWTH_MAX <- 12.5   # Manual maximum bound
...
POP_CHANGE_MIN <- -.1  # Manual minimum bound
POP_CHANGE_MAX <- .051  # Manual maximum bound
...
# Output file names
COVARIANCES_MAP_FILE <- "iso_growth_comm_bg.pdf"
...
avg_cov_inc_term_mean <- mean(map_data$cum_sel_inc_ratio, na.rm = TRUE)
avg_cov_gro_term_mean <- mean(map_data$cum_sel_gro_ratio, na.rm = TRUE)
...
income_growth_map <- create_variable_map(
  ...
  latex_title = expression(paste("Average Income Growth")),
  latex_legend = "X",
...
a_income_growth_map <- create_variable_map(
  ...
  "LogAvgIncInitial_tr",
  "Initial Incomes",
  latex_title = expression(paste("Initial Incomes")),
  latex_legend = "X",
