...
ANALYSIS_PRICE_FILEPATH = "results/null_model/price_analysis_migration_gravity.pkl"
...
def _perform_final_checks_and_filter(df, level_name, pop_initial_col, log_avg_inc_initial_col):
    ...
    # Handle invalid log-average income by replacing with 0, as requested.
    if log_avg_inc_initial_col in df.columns:
        nan_count = df[log_avg_inc_initial_col].isna().sum()
        if nan_count > 0:
            print(f"    Found {nan_count} rows with invalid '{log_avg_inc_initial_col}'. Replacing with 0 instead of dropping.")
            df[log_avg_inc_initial_col].fillna(0, inplace=True)

    # Continue to filter out units with invalid or zero initial population.
    if pop_initial_col in df.columns:
        # First, drop rows where population itself is NaN
        df.dropna(subset=[pop_initial_col], inplace=True)
        # Then, filter out rows where population is not positive
        df = df[df[pop_initial_col] > 0]
    
    print(f"    Filtered {level_name} data: {len(df)} rows remain after cleaning (from {original_rows}).")
    return df
