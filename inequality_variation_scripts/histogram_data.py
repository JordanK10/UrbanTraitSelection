...
def create_histogram_with_fit(data, var_name, level, figsize=(4.3254, 4), n_bins=None):
    ...
    plt.tight_layout()
    plt.savefig(f'histogram_plots/{filename}', dpi=300, bbox_inches='tight', facecolor='white')
    plt.close()
    print(f"Saved: histogram_plots/{filename}")
...
def create_all_histograms(data):
    ...
    import os
    os.makedirs('histogram_plots', exist_ok=True)
...
def save_fit_results_table(fit_results):
    ...
    # Save to CSV
    results_df.to_csv('histogram_plots/distribution_fit_results.csv', index=False)
    print(f"\nSaved fit results to: histogram_plots/distribution_fit_results.csv")
...
def main():
    ...
    print(f"Generated {total_plots} histogram plots in histogram_plots/ directory")
    ...
    print("\nFit results saved to: histogram_plots/distribution_fit_results.csv")
