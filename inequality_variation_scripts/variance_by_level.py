...
def create_line_plot_with_confidence(stats_data, figsize=(4.5, 1.75)):
    ...
    # ax.set_ylabel('Aggregation Level', fontsize=12)
...
def save_plot(fig, filename='variance_by_level.pdf'):
    """Save the plot."""
...
def main():
    ...
    os.makedirs('variance_plots', exist_ok=True)
    ...
    if fig is not None:
        ...
        print("- Confidence intervals: Mean ± standard deviation")
        print("- Sample sizes shown below x-axis")
        print("Plot saved as: variance_plots/growth_rates_by_level.pdf")
    else:
