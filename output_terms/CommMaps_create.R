...
join_chicago_community_data <- function(comm_data, shape_data) {
  ...
  # If all else fails
  cat("\nCould not join the datasets effectively using name matching. Please verify the data.\n")
  cat("Any community areas not matched will not have data displayed.\n")
  
  # Create the final mapping using exact matches first, then fuzzy for the rest.
  final_mapping <- data.frame(
    std_community = shape_data$std_community,
    best_match = shape_data$matching_unitname, # From exact match attempt
    stringsAsFactors = FALSE
  )

  # Identify rows that need fuzzy matching
  rows_to_fuzzy_match <- is.na(final_mapping$best_match)
  communities_to_fuzzy_match <- final_mapping$std_community[rows_to_fuzzy_match]

  if(any(rows_to_fuzzy_match)) {
      cat("Attempting fuzzy match for", sum(rows_to_fuzzy_match), "unmatched communities.\n")
      # This uses the stringdist package for fuzzy matching
      # If not installed: install.packages("stringdist")
      library(stringdist)
      ...
      # Get fuzzy matches only for the ones that need it
      fuzzy_matches <- sapply(communities_to_fuzzy_match, find_best_match, candidates = comm_data$std_unitname)
      
      # Update the final_mapping table
      final_mapping$best_match[rows_to_fuzzy_match] <- fuzzy_matches
  }
  
  # Now join the shape data with our best effort mapping
  shape_with_final_map <- shape_data %>%
      left_join(final_mapping, by = "std_community")

  # And finally, join with the community data
  final_joined_data <- shape_with_final_map %>%
    left_join(comm_data, by = c("best_match" = "std_unitname"))

  key_var_check <- setdiff(names(comm_data), c("UnitName", "std_unitname", "row_id"))[1]
  final_match_count <- sum(!is.na(final_joined_data[[key_var_check]]))
  cat("\nFinal join after combining exact and fuzzy matching resulted in", final_match_count, "matched areas out of", nrow(shape_data), "\n")
...
INCOME_GROWTH_MIN <- -0.045  # Manual minimum bound
INCOME_GROWTH_MAX <- 0.055   # Manual maximum bound
...
A_INCOME_GROWTH_MIN <- -0.06  # Manual minimum bound
A_INCOME_GROWTH_MAX <- 0.05   # Manual maximum bound
...
POP_CHANGE_MIN <- -.04  # Manual minimum bound
POP_CHANGE_MAX <- .04  # Manual maximum bound
...
# NEW: Empirical Growth Map Settings
EMP_GROWTH_USE_DATA_DRIVEN_LIMITS <- FALSE  # Set to FALSE to use manual limits
EMP_GROWTH_USE_PERCENTILE_LIMITS <- FALSE   # When TRUE, uses 10-90th percentiles; when FALSE uses min-max
EMP_GROWTH_MIN <- -0.05  # Manual minimum bound
EMP_GROWTH_MAX <- 0.05   # Manual maximum bound
EMP_GROWTH_AUTO_COLOR_SCHEME <- FALSE  # When TRUE, selects inferno if min > 0, custom scheme otherwise
EMP_GROWTH_FORCE_INFERNO <- FALSE     # Force using inferno color scheme regardless of min value
EMP_GROWTH_FORCE_CUSTOM <- TRUE      # Force using custom color scheme regardless of min value
...
# Output file names
DEMOGRAPHICS_MAP_FILE <- "output_maps/chicago_community_maps_demosRel.pdf"
COVARIANCES_MAP_FILE <- "output_maps/chicago_community_maps_covars.pdf"
...
# NEW: Empirical Growth limits
if(EMP_GROWTH_USE_DATA_DRIVEN_LIMITS) {
  cat("Using data-driven limits for Empirical Growth Rate\n")
  # Use map-specific percentile setting
  USE_PERCENTILE_LIMITS_TEMP <- EMP_GROWTH_USE_PERCENTILE_LIMITS
  
  emp_growth_limits <- analyze_data_distribution(
    map_data, 
    "RelAvgG_emp_cm", 
    additional_data, 
    "RelAvgG_emp_cm"
  ...
  cat("Using manual limits for Empirical Growth Rate\n")
  emp_growth_limits <- list(min = EMP_GROWTH_MIN, max = EMP_GROWTH_MAX)
}
...
cat("Empirical Growth: min =", emp_growth_limits$min, ", max =", emp_growth_limits$max, "\n")
...
required_vars <- c("RelPopG_cm", "RelAvgG_emp_cm", "RelAvgG_pop_cm", "RelAvgG_inc_cm" )
...
frac_pop_change_map <- create_variable_map(
  map_data, 
  "RelPopG_cm",
  "Relative Household Growth",
  latex_title = expression(paste("Relative Household Growth")),
  latex_legend = "X",
...
# Create the empirical growth map
emp_growth_map <- create_variable_map(
  map_data, 
  "RelAvgG_emp_cm",
  "Relative Empirical Income Growth",
  latex_title = expression(paste("Relative Empirical Income Growth")),
  latex_legend = "X",
  max_limit = emp_growth_limits$max,
  min_limit = emp_growth_limits$min,
  auto_color_scheme = EMP_GROWTH_AUTO_COLOR_SCHEME,
  force_inferno = EMP_GROWTH_FORCE_INFERNO,
  force_custom = EMP_GROWTH_FORCE_CUSTOM,
  cap_values = TRUE,
  optimize_distribution = EMP_GROWTH_USE_DATA_DRIVEN_LIMITS
)
...
income_growth_map <- create_variable_map(
  map_data, 
  "RelAvgG_pop_cm",
  "Relative Population-Averaged Income Growth",
  latex_title = expression(paste("Relative Population-Averaged Income Growth")),
  latex_legend = "X",
...
a_income_growth_map <- create_variable_map(
  map_data, 
  "RelAvgG_inc_cm",
  "Relative Income-Averaged Income Growth",
  latex_title = expression(paste("Relative Income-Averaged Income Growth")),
  latex_legend = "X",
...
combined_map <-  frac_pop_change_map + emp_growth_map + income_growth_map +a_income_growth_map+
