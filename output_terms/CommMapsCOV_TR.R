...
if(!is.null(chicago_areas) && !is.null(community_col)) {
  ...
  if(nrow(community_data) > 0) {
    ...
    cat("ERROR: No community data found with Agg == 'cm'\n")
  }
} else {
...
if(nrow(community_data) > 0) {
  ...
  stop("No community data available - check that data file contains Agg == 'cm' records")
}
...
COVARIANCE_MIN_GRO <- -15  # Manual minimum bound
COVARIANCE_MAX_GRO <- 15  # Manual maximum bound
...
COVARIANCE_MIN_INC <- -30  # Manual minimum bound
COVARIANCE_MAX_INC <- 10   # Manual maximum bound
...
COVARIANCES_MAP_FILE <- "output_maps/chicago_community_maps_covars_tr.pdf"
...
required_vars <- c("Sel_cm_from_tr_inc_PNC_st",   "Sel_cm_from_tr_pop_PNC_st" )
...
price_cov_term_map <- create_variable_map(
  map_data, 
  "Sel_cm_from_tr_pop_PNC_st", 
  "Population Selection Effects, Tracts (% of Total Growth)",
  latex_title = expression(paste("Population Selection Effects, Tracts (% of Total Growth)")),
  latex_legend = "X",
...
income_cov_term_map <- create_variable_map(
  map_data, 
  "Sel_cm_from_tr_inc_PNC_st", 
  "Income Selection Effects, Tracts (% of Total Growth)",
  latex_title = expression(paste("Income Selection Effects, Tracts (% of Total Growth)")),
  latex_legend = "X",
