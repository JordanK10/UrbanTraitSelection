...
INCOME_GROWTH_MIN <- -0.015  # Manual minimum bound
INCOME_GROWTH_MAX <- 0.09   # Manual maximum bound
...
A_INCOME_GROWTH_MIN <- -0.03  # Manual minimum bound
A_INCOME_GROWTH_MAX <- 0.09   # Manual maximum bound
...
POP_CHANGE_MIN <- -0.05  # Manual minimum bound
...
cat("Average Income Growth: min =", income_growth_limits$min, ", max =", income_growth_limits$max, "\n")
cat("AggregateIncome Growth: min =", a_income_growth_limits$min, ", max =", a_income_growth_limits$max, "\n")
...
required_vars <- c("PopG_cm",   "AvgG_cm", "AggG_cm" )
...
income_growth_map <- create_variable_map(
  ...
  latex_title = expression(paste("Average Income Growth Rate")),
  latex_legend = "X",
...
aggregate_income_growth_map <- create_variable_map(
  map_data, 
  "AggG_cm",
  "Aggregate Growth Rate",
  latex_title = expression(paste("Aggregate Income Growth Rate")),
  latex_legend = "X",
...
frac_pop_change_map <- create_variable_map(
  map_data, 
  "PopG_cm",
  "Population Growth Rate (Household)",
  latex_title = expression(paste("Population Growth Rate (Household)")),
  latex_legend = "X",
...
combined_map <- frac_pop_change_map + income_growth_map +  aggregate_income_growth_map+
