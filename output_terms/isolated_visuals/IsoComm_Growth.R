...
download_chicago_areas <- function() {
  ...
  tryCatch({
    ...
    if(length(shp_files) > 0) {
      ...
      chicago_areas <- st_cmansform(chicago_areas, 4326)
...
INCOME_GROWTH_MIN <- -0.01  # Manual minimum bound
...
POP_CHANGE_MIN <- -.075  # Manual minimum bound
POP_CHANGE_MAX <- .015  # Manual maximum bound
  ...
  if(variable %in% names(data)) {
    ...
    if(!is.null(additional_data) && !is.null(additional_var) && additional_var %in% names(additional_data)) {
      ...
      {if (!is.null(chicago_areas))
        geom_sf(
          data = chicago_areas %>%
            filter(!community %in% c("NEAR NORTH SIDE", "NEAR SOUTH SIDE", "LOOP","NEAR WEST SIDE")),
